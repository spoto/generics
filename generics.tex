...
By using class \<Offer>, the \<SharedEntity> interface specifies three methods.
...
program explicit dynamic checks in code, as shown in a moment.
...
Let us state an important property about this code:

\begin{tcolorbox}
  \begin{center}\emph{Consistency of Shareholders}\end{center}
  If \<sse> is a {\codesize\texttt{SimpleSharedEntity<S,O>}}
  then the domain of its map \<shares> consists of objects of type \<S> only.
\end{tcolorbox}

\noindent
It turns out that this property holds.
Namely, the code does not use unchecked casts, hence it is strongly-typed and
...
not strongly-typed. Therefore, also contracts not of type \<S> could call \<accept>
