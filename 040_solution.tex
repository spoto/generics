...
compilation of generics, restricted to a specific method and forced manually, rather than
automatically performed by the compiler. It is interesting to consider which methods
would need that redefinition, in general. They are those that have a parameter of generic type and
that generic type is restricted in the subclass. For instance, method \<accept> in
...
parameters. In general, a compiler might perform this automatically or a static analysis
tool might issue a warning when such redefinition is needed. Currently, however, this is
left to the programmer, which might be the origin of security issues, as shown
in Sec.~\ref{sec:attack}.
